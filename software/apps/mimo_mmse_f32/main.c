...
float s[2 * N_TX * N_ITR]   __attribute__((section(".l1_prio")));
float x[2 * N_TX * N_ITR]   __attribute__((section(".l1_prio")));
float y[2 * N_TX * N_ITR]   __attribute__((section(".l1_prio")));
#endif
...
void parallel_mimo_mmse_cholesky() {
  ...
  for (uint32_t itr = pool_id; itr < N_ITR; itr += num_pools) {
    ...
    mempool_hermitian_folded_f32p(ch_matrix_ptr, in_matrix_ptr, sigma_ptr, N_RX, N_TX, core_id % N_TX, N_TX);
  }
  mempool_stop_benchmark();
  mempool_start_benchmark();
  for (uint32_t itr = core_id; itr < N_ITR; itr += num_cores) {
    ...
    mempool_MVP_conjtransp_folded_f32s(ch_matrix + itr * (2 * N_TX * N_RX), b + itr * (2 * N_RX), s_ptr, N_RX, N_TX);
  ...
  for (uint32_t itr = pool_id; itr < N_ITR; itr += num_pools) {
    ...
    float* sigma_ptr = sigma + N_TX * itr;
    mempool_hermitian_f32p(ch_matrix_ptr, in_matrix_ptr, sigma_ptr, N_RX, N_TX, core_id % N_TX, N_TX);
  }
  mempool_stop_benchmark();
  mempool_start_benchmark();
  for (uint32_t itr = core_id; itr < N_ITR; itr += num_cores) {
    mempool_MVP_conjtransp_f32s(ch_matrix + itr * 2*N_TX*N_RX, b + itr * 2*N_RX, s + itr * 2*N_TX, N_RX, N_TX);
    mempool_cholesky_f32s(in_matrix + itr * 2*N_TX*N_TX, out_matrix + itr * 2*N_TX*N_TX, N_TX);
    mempool_Ltrisol_f32s(out_matrix + itr * 2*N_TX*N_TX, s + itr * 2*N_TX, y + itr * 2*N_TX, N_TX);
  ...
  // verify_result(s, Out_s, N_TX, core_id);
  // verify_result(y, Out_y, N_TX, core_id);
  verify_result(x, Out_x, N_TX*N_ITR, core_id);
