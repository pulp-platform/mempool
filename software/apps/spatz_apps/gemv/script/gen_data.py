...
def emit_gemv_layer(name="gemv", **kwargs):
    ...
    layer_str += f'#define PREC {kwargs["prec"]}' + '\n\n'
    layer_str += f'static uint32_t active_cores = {cores}' + ';\n\n'
    layer_str += f'static uint32_t multiB = {multiB}' + ';\n\n'

    layer_str += f'{dtype} a[{m} * {n}] __attribute__((section(".l1_prio")))' + ';\n'
    layer_str += f'{dtype} b[{multiB}][{n}] __attribute__((section(".l1_prio")))' + ';\n'
    layer_str += f'{dtype} result[{m}] __attribute__((section(".l1_prio")))' + ';\n\n\n'
