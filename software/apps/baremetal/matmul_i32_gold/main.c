...
void verify_matrix(int32_t *__restrict__ pRes, int32_t *__restrict__ pExp, uint32_t NEL, 
...
int main() {
  ...
  if (core_id == 0) {
    ...
    dma_memcpy_blocking(matric_c_gold, C, matrix_M * matrix_P * sizeof(int32_t));
  }
  mempool_barrier(num_cores);

  mempool_start_benchmark();
  mat_mul_unrolled_4x4_parallel_asm(matrix_a, matrix_b, matrix_c, matrix_M, matrix_N, matrix_P, core_id, num_cores);
  ...
  verify_matrix(matrix_c, matric_c_gold, matrix_M * matrix_P, core_id, num_cores);
  mempool_barrier(num_cores);
