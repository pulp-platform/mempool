...
#ifdef FOLDED_TWIDDLES
static int16_t *
mempool_cfft_q16p_folded(uint16_t fftLen, int16_t *pCoef_src,
...
#else
static int16_t *
mempool_cfft_q16p_folded(uint16_t fftLen, int16_t *pCoef,
                         uint16_t *pBitRevTable, int16_t *pSrc16,
                         int16_t *pDst16, uint16_t bitReverseLen,
                         uint8_t ifftFlag, uint8_t bitReverseFlag, uint32_t nPE)
#endif
{
  ...
  if (ifftFlag == 0) {
#ifdef FOLDED_TWIDDLES
    pTmp = mempool_butterfly_q16p_folded(pSrc16, pDst16, fftLen, pCoef_src,
                                         pCoef_dst, nPE);
#else
  ...
  if (bitReverseFlag) {
    ...
    mempool_bitrev_q16p_xpulpimg((uint16_t *)pSrc16, (uint16_t *)pDst16, fftLen,
                                 nPE);
#else
    if (pTmp == pSrc16)
      pDst16 = pSrc16;
