...
void mempool_bitreversal_memsized_q16p( uint16_t *pSrc, const uint16_t bitRevLen, const uint16_t *pBitRevTab, const uint32_t nPE);
...
#endif
{
    ...
    if (bitReverseFlag) {
      mempool_bitreversal_q16p((uint16_t *)pSrc16, bitReverseLen, pBitRevTable, nPE);
    }
...
#endif
{
    ...
    for(i0 = core_id * 4; i0 < MIN(core_id * 4 + 4, n2); i0++) {
        ...
        *((v2s *)&pDst16[i1_store * 2U]) = __PACK2(t0, t1);
        *((v2s *)&pDst16[i2_store * 2U]) = __PACK2(t2, t3);
        *((v2s *)&pDst16[i3_store * 2U]) = __PACK2(t4, t5);
