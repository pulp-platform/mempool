...
#if defined(PARALLEL)
    int32_t LL_matrix[N_ROW * N * N_BANKS]     __attribute__((aligned(N_BANKS), section(".l1")));
    int32_t LR_matrix[N_ROW * N * N_BANKS]     __attribute__((aligned(N_BANKS), section(".l1")));
    int32_t In_matrix[N * N_BANKS]     __attribute__((aligned(N_BANKS), section(".l1")));
#else
...
void parallel() {
    ...
    if (core_id < N_COL * nPE) {
        mempool_start_benchmark();
        mempool_cholesky_q32p_fold(In_matrix, In_matrix, LL_matrix, LR_matrix, N, N_ROW, N_COL, FIXED_POINT, nPE);
