...
// Initialize the Twiddles folded
  ...
  for (uint32_t j = 0; j < N_FFTs_COL; j++) {
    ...
    for (uint32_t i = core_id; i < N_WORDS_COL; i += num_cores) {
      *(v2s *)&l1_twiddleCoef_q16_src[2U * (i + j * N_WORDS_COL)] =
          *(v2s *)&l2_twiddleCoef_q16[2U * i];
      *(v2s *)&l1_twiddleCoef_q16_src[2U *
  ...
  if (core_id < N_FFTs_COL * nPE) {
    ...
    mempool_radix4_cfft_q16p_scheduler(
        ...
        l1_twiddleCoef_q16_src + 2 * col_id * N_WORDS_COL,
        l1_twiddleCoef_q16_dst + 2 * col_id * N_WORDS_COL, l1_BitRevIndexTable,
