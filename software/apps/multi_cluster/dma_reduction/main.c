 ...
 * sum.
 *
 * Exercises MC_DMA_MODE_REDUCE (DMMASK + DMCPYC, collective_type = 2 =
...
static uint16_t l1_result[TRANSFER_ELEMS]
    __attribute__((section(".l1"), aligned(64)));
...
int main() {
  ...
  if (mc_is_dm_core() && cid == 0) {
    ...
    mc_dma_async_reduction(L1_OFFSET(l1_result), L1_OFFSET(l1_src),
                           TRANSFER_BYTES, COLLECTIVE_REDADD_UINT_16,
                           REDUCE_MASK_ALL, REDUCE_MASK_ALL);
    mc_dma_async_wait_all();
      ...
      if (l1_result[i] != expected) {
        if (errors < 4)
          printf("  mismatch[%d]: got 0x%04x, expected 0x%04x\n", i,
                 l1_result[i], expected);
        errors++;
      }
    }
    printf(
        "[Reduction] Verification: %s (%u errors out of %u, %u contributors)\n",
        errors == 0 ? "PASS" : "FAIL", errors, TRANSFER_ELEMS, n_contributors);
    eoc_val = errors;
  }
