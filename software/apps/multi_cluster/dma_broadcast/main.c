 ...
 *   - Broadcast fan-out across the full ARCH_NUM_CLUSTER_X x ARCH_NUM_CLUSTER_Y
 * mesh
 */
 ...
 *   in-between → pin specific bits    → coset of source (e.g. 0x0001 = same
 * parity) */
#define BCAST_MASK_ALL 0x0000u /* whole axis: every column / every row   */
#define BCAST_MASK_SELF_ONLY                                                   \
  0xFFFFu /* exact coordinate match: just one slice */

/* L1 buffers replicated on every cluster */
static uint16_t l1_src[TRANSFER_ELEMS]
    __attribute__((section(".l1"), aligned(64)));
static uint16_t l1_dst[TRANSFER_ELEMS]
    __attribute__((section(".l1"), aligned(64)));
...
int main() {
  ...
  if (mc_is_dm_core() && cid == 0) {
    printf("[Broadcast] Mesh: %dx%d clusters, %u elems, %u bytes/transfer\n",
           ARCH_NUM_CLUSTER_X, ARCH_NUM_CLUSTER_Y, TRANSFER_ELEMS,
    ...
    bare_dma_start_1d_broadcast((uint64_t)remote_pos(self_pos, dst_offset),
                                (uint64_t)local(src_offset), TRANSFER_BYTES,
                                row_mask, col_mask);
    mc_dma_async_wait_all();
  }
  ...
  if (mc_is_dm_core() && cid == 0) {
    ...
    for (uint32_t rid = 1; rid < ARCH_NUM_CLUSTER; ++rid) {
      mc_dma_async_1d((uint64_t)(uintptr_t)&remote_err,
    ...
    printf("[Broadcast] Verification: %s "
           ...
           total_errors == 0 ? "PASS" : "FAIL", total_errors, failing_clusters,
           (uint32_t)ARCH_NUM_CLUSTER);
