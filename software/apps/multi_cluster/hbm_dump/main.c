...
static uint16_t l1_dma_buffer[HBM_TRANSFER_ELEMS]
...
int main() {
  ...
  if (mc_is_dm_core() && (mc_get_cluster_id() == 0)) {
    ...
    printf("[Before load HBM to L1] the first 8 elements of local L1 buffer "
           "are:\n");
    for (int i = 0; i < 8; ++i) {
      printf("    0x%04x\n", local_ptr[i]);
    }

    mc_dma_async_1d((uint64_t)(uintptr_t)l1_dma_buffer,
                    hbm_addr(A_matrix_in_HBM_offset), transfer_size);
    printf("[Now    load HBM to L1] loading with asynchronize API\n");
    mc_dma_async_wait_all();

    printf("[After  load HBM to L1] the first 8 elements of local L1 buffer "
           "are:\n");
    for (int i = 0; i < 8; ++i) {
      printf("    0x%04x\n", local_ptr[i]);
    }
    ...
    mc_dma_async_1d(hbm_addr(destination_in_HBM_offest),
                    (uint64_t)(uintptr_t)l1_dma_buffer, transfer_size);
