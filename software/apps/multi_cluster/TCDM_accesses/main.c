...
static uint32_t l1_src_pattern[NUM_ELEMS]
    __attribute__((section(".l1"), aligned(64)));
static uint32_t l1_remote_results[ARCH_NUM_CLUSTER + 2][NUM_ELEMS]
    __attribute__((section(".l1"), aligned(64)));
...
int main() {
  ...
  if (mc_is_dm_core()) {
    ...
    if (cid == 0) {
      printf("[Cluster %d] Wrote local pattern: 0x%08x .. 0x%08x\n", cid,
             local_ptr[0], local_ptr[NUM_ELEMS - 1]);
    }
  }
  ...
  if (mc_is_dm_core() && cid == 0) {
    ...
    uint32_t src_offset =
        (uint32_t)((uintptr_t)l1_src_pattern - (uintptr_t)local(0));

    for (uint32_t remote_id = 0; remote_id < ARCH_NUM_CLUSTER; ++remote_id) {
      /* DMA: remote cluster TCDM -> local L1 result buffer */
      volatile uint32_t *buf =
          (volatile uint32_t *)l1_remote_results[remote_id];
      mc_dma_async_1d((uint64_t)(uintptr_t)buf,
                      (uint64_t)remote_cid(remote_id, src_offset),
                      transfer_size);
      mc_dma_async_wait_all();

      printf("[Cluster 0] Read from cluster %d (remote_cid): first=0x%08x "
             "last=0x%08x\n",
             remote_id, buf[0], buf[NUM_ELEMS - 1]);
    }

    /* Also demonstrate remote_xy: read cluster at (1,0) */
    volatile uint32_t *xy_buf =
        (volatile uint32_t *)l1_remote_results[ARCH_NUM_CLUSTER];
    mc_dma_async_1d((uint64_t)(uintptr_t)xy_buf,
                    (uint64_t)remote_xy(1, 0, src_offset), transfer_size);
    mc_dma_async_wait_all();
    ...
    target_pos.x = 0;
    target_pos.y = 1;
    volatile uint32_t *pos_buf =
        (volatile uint32_t *)l1_remote_results[ARCH_NUM_CLUSTER + 1];
    mc_dma_async_1d((uint64_t)(uintptr_t)pos_buf,
