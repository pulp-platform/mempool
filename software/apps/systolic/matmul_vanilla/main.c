...
#define DIM_P 96
#else
#define DIM_M (NUM_CORES)
#define DIM_N (NUM_CORES)
#define DIM_P (NUM_CORES)
#endif

int32_t matrix_a[DIM_M * DIM_N] __attribute__((section(".l1_prio")));
int32_t matrix_b[DIM_N * DIM_P] __attribute__((section(".l1_prio")));
int32_t matrix_c[DIM_M * DIM_P] __attribute__((section(".l1_prio")));
...
int main() {
  ...
  test_matrix_multiplication(matrix_a, matrix_b, matrix_c, DIM_M, DIM_N, DIM_P,
                             core_id, num_cores);
