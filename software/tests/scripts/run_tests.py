    ...
    try:
        ...
        with subprocess.Popen(
            ...
            stdout_thread = threading.Thread(
                target=enqueue_output, args=(p.stdout, stdout_queue)
            )
            stderr_thread = threading.Thread(
                target=enqueue_output, args=(p.stderr, stderr_queue)
            )
                    ...
                    for line in [stdout_line, stderr_line]:
                        if line:
                            ...
                            if ("Error 117") in line:
                                print(
                                    f"{RED}[FAIL]{RESET}: Banshee called "
...
def main():
    ...
    for test in sorted(set(matching_tests)):
        ...
        if args.compiler and not compile_test(test, args):
            continue
