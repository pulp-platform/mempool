...
void gemv_f32p_local_unrolled4(float *in_a, float *in_x, float *y,
                               uint32_t N, uint32_t P) {

  uint32_t core_id = mempool_get_core_id();
  uint32_t num_cores = mempool_get_core_count();
  uint32_t const parallel_factor = 32;              // Number of rows processed in parallel per outer loop
  uint32_t const remainder = N % BANKING_FACTOR;    // Remaining elements in each row, to be processed in the end with core 0
  ...
  for (uint32_t i_start = 0; i_start < P; i_start += parallel_factor) {
    ...
    if (i < P) {
      ...
      uint32_t j_offset = (core_id % parallel_factor) * BANKING_FACTOR;   // Reduce bank conflicts
      uint32_t j_block = parallel_factor * BANKING_FACTOR;                // Number of elements per inner loop
      for (uint32_t j_start = (core_id / parallel_factor) * parallel_factor * BANKING_FACTOR;
      ...
      while (red_cores > 1) {
        ...
        if (__atomic_fetch_add(&red_barrier[idx + (previous_step - 1) * BANKING_FACTOR + j_offset], 1,
                               __ATOMIC_RELAXED)) {
    
          // Reduction
          float add = sum[idx + previous_step * BANKING_FACTOR + j_offset];
          asm volatile("fadd.s %0, %0, %1;" : "+&r"(sum[idx + j_offset]) : "r"(add));
    
          // Next level of binary tree
          __atomic_store_n(&red_barrier[idx + (previous_step - 1) * BANKING_FACTOR + j_offset], 0,
                           __ATOMIC_RELAXED);
