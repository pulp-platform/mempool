...
void matmul_4x4_parallel_f16p_nocopt_asm(
    __fp16 const *__restrict__ A, __fp16 const *__restrict__ B,
    __fp16 *__restrict__ C, uint32_t M, uint32_t N, uint32_t P, uint32_t id,
    uint32_t numThreads) {
  ...
  // Inner Loop Control
  // k_offset = (Core offset) + (Window offset) + (Group offset from MatrixB)
  uint32_t k_offset = 2 * ((id % c) * (tile_core_nums * BANKING_FACTOR + 1) + id / c * 2 + \
  ...
  uint32_t j_offset = id / c / j_conflict_tile * tile_core_nums * BANKING_FACTOR * 2 + id / (tile_nums / 4) * 4;
  /////////////////////////////
  //      LOOP  CONTROL      //
  /////////////////////////////
  // Inner Round-Robin
    ...
    for (uint32_t j = j_offset_counter; j < P_counter; j += 4) {
      ...
      __asm__ volatile(
          ...
          : [addr_a] "+&r"(addr_a), [addr_b] "+&r"(addr_b),
            [addr_a_ori] "+&r"(addr_a_ori), [addr_b_ori] "+&r"(addr_b_ori), // Outputs
            [x1] "+r"(k)
          : [N3_1] "r"(_N31), [P_1] "I"(_P1), [N2] "I"(_N2), [P2] "I"(_P2), [P2_1] "I"(_P21) // Inputs
          : "x3", "x4", "x10", "x11", "x12", "x13", "x14", "x15",
            "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23",
            "x24", "x25", "x26", "x27", "x28", "x29", "x30", "x31", "memory"); // Clobber
    }
    if (j_offset_counter != c_start) {
      P_counter = j_offset_counter;
      j_offset_counter = c_start;
