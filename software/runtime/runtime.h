...
static uint32_t volatile *wake_up_cluster_reg =
    (uint32_t volatile *)(CONTROL_REGISTER_OFFSET +
                          CONTROL_REGISTERS_WAKE_UP_CLUSTER_REG_OFFSET);
...
/// Obtain the number of clusters in the current cluster.
static inline uint32_t mempool_get_cluster_count() { return NUM_CLUSTERS; }

/// Obtain the ID of the cluster the current core is in.
static inline uint32_t mempool_get_cluster_id() {
  return mempool_get_core_id() / (NUM_CORES_PER_CLUSTER);
}
