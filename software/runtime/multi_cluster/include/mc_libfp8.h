...
void spatz_verify(uint32_t avl, uint8_t *act_vec, const uint8_t *exp_vec,
                  const float tol) {
  ...
  for (uint32_t i = 0; i < avl; i++) {
    ...
    if (err > tol || err < -tol) {
      printf("%3d | exp: 0x%02x (%.5f), act: 0x%02x (%.5f), err: %.5f\n", i,
             exp_fp8, exp_f32, act_fp8, act_f32, err);
      tot_err += 1;
    }
  }
...
void spatz_verify_16(uint32_t avl, uint16_t *act_vec, const uint16_t *exp_vec,
                     const float tol) {
  ...
  for (uint32_t i = 0; i < avl; i++) {
    ...
    if (err > tol || err < -tol) {
      printf("%3d | exp: 0x%04x (%.5f), act: 0x%04x (%.5f), err: %.5f\n", i,
             exp_fp16, exp_f32, act_fp16, act_f32, err);
      tot_err += 1;
    }
  }
