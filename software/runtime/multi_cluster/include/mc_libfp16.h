...
fp16 float_to_fp16(float value) {
  ...
  int32_t exponent =
      floatBits.parts.exponent - 127 + 15; // adjust bias from 127 to 15
  uint32_t mantissa = floatBits.parts.mantissa >> 13; // reduce to 10 bits

  if (exponent <= 0) {
    if (exponent < -10)
      return sign; // too small
...
void matmul_fp16(fp16 *z, fp16 *y, fp16 *x, fp16 *w, uint16_t m_size,
                 uint16_t n_size, uint16_t k_size) {
  for (int i = 0; i < m_size; ++i) {
    for (int j = 0; j < k_size; ++j) {
      z[i * k_size + j] = y[i * k_size + j];
      for (int k = 0; k < n_size; ++k) {
        z[i * k_size + j] =
            fp16_fma(x[i * n_size + k], w[k * k_size + j], z[i * k_size + j]);
      }
    }
  }
}
...
inline void asm_fp16_exp(const fp16 *inp, fp16 *oup) {
  ...
  asm volatile(
      ...
      : [r] "=r"(r) // Output
      : [x] "r"(x), [c1] "r"(c1), [cHalf] "r"(cHalf), [cOneSix] "r"(cOneSix),
        [cOne24] "r"(cOne24)
      : "ft0", "ft1", "ft2", "ft3");
...
inline void asm_fp16_sigmoid(const fp16 *inp, fp16 *out) {
  ...
  //----------------------------------------------------------------
  asm volatile("fmv.h.x    ft1, %[one]       \n" // ft1 = 1.0
               "fmv.h.x    ft2, %[eNegX]     \n" // ft2 = eNegX
               "fadd.h     ft1, ft1, ft2     \n" // ft1 = 1 + eNegX
               "fmv.x.h    %[sum], ft1       \n" // sum = (1 + eNegX)
               : [sum] "=r"(sum)
               : [one] "r"(ONE), [eNegX] "r"(eNegX)
               : "ft1", "ft2");
...
#define VFR_TYPE_ENCODE(funct6, m, vs2, vs1, funct3, vd, opcode)               \
  ((funct6 << 26) | (m << 25) | (vs2 << 20) | (vs1 << 15) | (funct3 << 12) |   \
   (vd << 7) | (opcode))

inline void asm_rvv_exp(uint32_t vs1_num, uint32_t vd_num) {
  asm volatile(".word %0\n" ::"i"(VFR_TYPE_ENCODE(
      0b001100, 0b1, 0b00000, vs1_num, 0b001, vd_num, 0b1010111)));
}
#endif
