...
static inline void mc_alloc_init(){
    ...
    if (mc_is_first_core()){
        mc_cluster_alloc_init(mc_get_allocator_hbm(), (void *)hbm_heap_start, hbm_heap_size);
    }
...
void mc_reset_barrier(uint32_t* barrier){
    *barrier = mc_get_disable_value();
}
...
void mc_intra_cluster_sync(){
    ...
    // [mempool_barrier_init() | mc_barrier_init() | mc_barrier_xy_init()] must be done once before first use.
