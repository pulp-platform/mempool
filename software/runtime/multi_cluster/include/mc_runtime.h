#ifndef _MC_RUNTIME_H_
#define _MC_RUNTIME_H_
#include "mc_alloc.h"
...
static inline void *simple_malloc(const uint32_t size) {
  return mc_l1_malloc(size);
}
...
#define cluster_index(x, y)                                                    \
  (((uint32_t)(y)) * ((uint32_t)ARCH_NUM_CLUSTER_X) + (x))
#define local(offset) (ARCH_CLUSTER_TCDM_BASE + (offset))
#define zomem(offset) (ARCH_CLUSTER_ZOMEM_BASE + (offset))
#define remote_cid(cid, offset)                                                \
  (ARCH_CLUSTER_TCDM_REMOTE + (cid)*ARCH_CLUSTER_TCDM_SIZE + (offset))
#define remote_xy(x, y, offset)                                                \
  (ARCH_CLUSTER_TCDM_REMOTE + cluster_index(x, y) * ARCH_CLUSTER_TCDM_SIZE +   \
   (offset))
#define remote_pos(pos, offset)                                                \
  (ARCH_CLUSTER_TCDM_REMOTE +                                                  \
   cluster_index((pos).x, (pos).y) * ARCH_CLUSTER_TCDM_SIZE + (offset))
#define hbm_addr(offset) ((uint64_t)ARCH_HBM_START_BASE + (uint64_t)(offset))
#define hbm_west(nid, offset)                                                  \
  ((uint64_t)ARCH_HBM_START_BASE +                                             \
   ((uint64_t)(nid)) * (uint64_t)ARCH_HBM_NODE_ADDR_SPACE +                    \
   (uint64_t)(offset))
#define hbm_north(nid, offset)                                                 \
  ((uint64_t)ARCH_HBM_START_BASE +                                             \
   ((uint64_t)(nid)) * (uint64_t)ARCH_HBM_NODE_ADDR_SPACE +                    \
   (uint64_t)ARCH_HBM_NODE_ADDR_SPACE * (uint64_t)ARCH_NUM_CLUSTER_Y +         \
   (uint64_t)(offset))
#define hbm_east(nid, offset)                                                  \
  ((uint64_t)ARCH_HBM_START_BASE +                                             \
   ((uint64_t)(nid)) * (uint64_t)ARCH_HBM_NODE_ADDR_SPACE +                    \
   (uint64_t)ARCH_HBM_NODE_ADDR_SPACE *                                        \
       ((uint64_t)ARCH_NUM_CLUSTER_Y + (uint64_t)ARCH_NUM_CLUSTER_X) +         \
   (uint64_t)(offset))
#define hbm_south(nid, offset)                                                 \
  ((uint64_t)ARCH_HBM_START_BASE +                                             \
   ((uint64_t)(nid)) * (uint64_t)ARCH_HBM_NODE_ADDR_SPACE +                    \
   (uint64_t)ARCH_HBM_NODE_ADDR_SPACE * 2 * (uint64_t)ARCH_NUM_CLUSTER_Y +     \
   (uint64_t)ARCH_HBM_NODE_ADDR_SPACE * ARCH_NUM_CLUSTER_X +                   \
   (uint64_t)(offset))
...
typedef struct McPosition {
  uint32_t x;
  uint32_t y;
} McPosition;

McPosition get_pos(uint32_t cluster_id) {
  McPosition pos;
...
// Methods
McPosition right_pos(McPosition pos) {
...
McPosition left_pos(McPosition pos) {
...
McPosition top_pos(McPosition pos) {
...
McPosition bottom_pos(McPosition pos) {
  uint32_t new_x = pos.x;
...
uint32_t mc_get_core_id() {
  uint32_t hartid;
  asm volatile("csrr %0, mhartid" : "=r"(hartid));
  return hartid;
}

uint32_t mc_is_dm_core() {
  uint32_t hartid;
  asm volatile("csrr %0, mhartid" : "=r"(hartid));
  return (hartid == ARCH_NUM_CORE_PER_CLUSTER - 1);
}

uint32_t mc_is_first_core() {
  uint32_t hartid;
  asm volatile("csrr %0, mhartid" : "=r"(hartid));
  return (hartid == 0);
}
...
static inline void mc_alloc_init() {
  uint32_t CID = mc_get_cluster_id();
  // volatile uint32_t * heap_start      = (volatile uint32_t *)
  // (ARCH_CLUSTER_HEAP_BASE + 0x1000);
  ...
  if (mc_is_first_core()) {
    mc_cluster_alloc_init(mc_get_allocator_l1(), (void *)heap_start, heap_size);
  }

  // HBM allocator
  uint32_t hbm_nodes = 4; // bowwang: hardcoded for now
  volatile uint32_t *hbm_heap_start = (volatile uint32_t *)__hbm_heap_start;
  volatile uint32_t *hbm_heap_end =
      (volatile uint32_t *)(ARCH_HBM_START_BASE +
                            (ARCH_HBM_NODE_ADDR_SPACE * hbm_nodes));
  volatile uint32_t hbm_heap_size =
      (uint32_t)hbm_heap_end - (uint32_t)hbm_heap_start;
  if (mc_is_first_core()) {
    mc_cluster_alloc_init(mc_get_allocator_hbm(), (void *)hbm_heap_start,
                          hbm_heap_size);
  }

  // allocation init summary
  if (CID == 0 && mc_is_first_core()) {
...
uint32_t mc_get_enable_value() {
  volatile uint32_t *amo_reg = (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 4);
  return *amo_reg;
}

uint32_t mc_get_disable_value() {
  volatile uint32_t *amo_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 24);
  return *amo_reg;
}

uint32_t mc_get_barrier_num_cluster() {
  volatile uint32_t *info_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 8);
  return *info_reg;
}

uint32_t mc_get_barrier_num_cluster_x() {
  volatile uint32_t *info_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 12);
  return *info_reg;
}

uint32_t mc_get_barrier_num_cluster_y() {
  volatile uint32_t *info_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 16);
  return *info_reg;
}

void mc_annotate_barrier(uint32_t type) {
  volatile uint32_t *info_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 20);
  *info_reg = type;
}

void mc_reset_barrier(uint32_t *barrier) { *barrier = mc_get_disable_value(); }

uint32_t mc_amo_fetch_add(uint32_t *barrier) {
  return __atomic_fetch_add((uint32_t *)barrier, mc_get_enable_value(),
                            __ATOMIC_RELAXED);
}
...
void mc_intra_cluster_sync() {
  ...
  // [mempool_barrier_init() | mc_barrier_init() | mc_barrier_xy_init()] must be
  // done once before first use.
...
inline void mc_wakeup_clusters(uint8_t row_mask, uint8_t col_mask) {
  volatile uint32_t *wakeup_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 28);
  uint32_t value = (((uint32_t)col_mask) << 16) | row_mask;
  *wakeup_reg = value;
}

inline void mc_wakeup_all_clusters() {
  volatile uint32_t *wakeup_reg =
      (volatile uint32_t *)(ARCH_CLUSTER_REG_BASE + 28);
  uint32_t value = mc_get_disable_value();
  *wakeup_reg = value;
}

void mc_barrier_init() {
#ifdef MEMPOOL_CLUSTER_UNIT
  // Initialize mempool barrier state once before using repeated intra-cluster
  // barriers.
...
void mc_global_barrier() {
  ...
  if (mc_is_dm_core()) {
    ...
    if ((mc_get_barrier_num_cluster() - mc_get_enable_value()) ==
        mc_amo_fetch_add((uint32_t *)barrier)) {
      mc_reset_barrier((uint32_t *)barrier);
      mc_wakeup_all_clusters();
    }
    *cluster_reg = mc_get_enable_value();
...
void mc_global_barrier_polling() {
  volatile uint32_t *barrier = (volatile uint32_t *)ARCH_SYNC_BASE;
...
void mc_barrier_xy_init() {
  ...
  volatile uint32_t *barrier_y =
      (volatile uint32_t *)(ARCH_SYNC_BASE +
                            (cluster_index(pos_x_middel, pos_y_middel) *
                             ARCH_SYNC_SIZE) +
                            16);
  volatile uint32_t *cluster_reg = (volatile uint32_t *)ARCH_CLUSTER_REG_BASE;
  ...
  if (mc_is_dm_core()) {
    if (mc_get_cluster_id() == 0) {
      ...
      for (int i = 0; i < ARCH_NUM_CLUSTER_Y; ++i) {
        volatile uint32_t *barrier_x =
            (volatile uint32_t *)(ARCH_SYNC_BASE +
                                  (cluster_index(pos_x_middel, i) *
                                   ARCH_SYNC_SIZE) +
                                  8);
...
void mc_global_barrier_xy() {
  ...
  if (mc_is_dm_core()) {
    ...
    uint32_t pos_x_middel = (mc_get_barrier_num_cluster_x()) / 2;
    uint32_t pos_y_middel = (mc_get_barrier_num_cluster_y()) / 2;
    volatile uint32_t *barrier_x =
        (volatile uint32_t *)(ARCH_SYNC_BASE +
                              (cluster_index(pos_x_middel, pos.y) *
                               ARCH_SYNC_SIZE) +
                              8);
    volatile uint32_t *barrier_y =
        (volatile uint32_t *)(ARCH_SYNC_BASE +
    ...
    // First Barrier X
    if ((mc_get_barrier_num_cluster_x() - mc_get_enable_value()) ==
        mc_amo_fetch_add((uint32_t *)barrier_x)) {
      mc_reset_barrier((uint32_t *)barrier_x);

      // For cluster synced X, then sync Y
      if ((mc_get_barrier_num_cluster_y() - mc_get_enable_value()) ==
          mc_amo_fetch_add((uint32_t *)barrier_y)) {
        mc_reset_barrier((uint32_t *)barrier_y);
        mc_wakeup_all_clusters();
      }
    }
...
void mc_global_barrier_xy_polling() {
...
void mc_timer_start() {
  volatile uint32_t *start_reg =
      (volatile uint32_t *)(ARCH_SOC_REGISTER_EOC + 8);
  *start_reg = mc_get_enable_value();
}

void mc_timer_end() {
  volatile uint32_t *end_reg =
      (volatile uint32_t *)(ARCH_SOC_REGISTER_EOC + 12);
  *end_reg = mc_get_enable_value();
}
...
void mc_print(char *str) {
  for (int i = 0; str[i] != '\0'; i++) {
    mc_log_char(str[i]);
  }
}

void mc_print_int(uint32_t data) {
  volatile uint32_t *log_reg =
      (volatile uint32_t *)(ARCH_SOC_REGISTER_EOC + 20);
  *log_reg = data;
}
...
void mc_sat(uint32_t val) {
  volatile uint32_t *sat_reg =
      (volatile uint32_t *)(ARCH_SOC_REGISTER_EOC + 40);
  *sat_reg = val;
}
