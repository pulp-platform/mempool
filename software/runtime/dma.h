...
static inline void dma_config(uint32_t cluster_id, bool decouple, bool deburst, bool serialize) {
  volatile uint32_t *_dma_conf_reg =
      (volatile uint32_t *)(DMA_BASE(cluster_id) + MEMPOOL_DMA_FRONTEND_CONF_REG_OFFSET);
  // Configure the DMA
  uint32_t config = 0;
...
static inline uint32_t dma_idle(uint32_t cluster_id) {
  volatile uint32_t *_dma_status_reg =
      (volatile uint32_t *)(DMA_BASE(cluster_id) + MEMPOOL_DMA_FRONTEND_STATUS_REG_OFFSET);
...
static inline uint32_t dma_done(uint32_t cluster_id) {
  volatile uint32_t *_dma_done_reg =
      (volatile uint32_t *)(DMA_BASE(cluster_id) + MEMPOOL_DMA_FRONTEND_DONE_REG_OFFSET);
...
void dma_memcpy_nonblocking(uint32_t cluster_id, void *dest, const void *src, size_t len) {
  ...
  volatile uint32_t *_dma_id_reg =
      (volatile uint32_t *)(DMA_BASE(cluster_id) + MEMPOOL_DMA_FRONTEND_NEXT_ID_REG_OFFSET);
  // Configure the DMA
...
void dma_memcpy_blocking(uint32_t cluster_id, void *dest, const void *src, size_t len) {
