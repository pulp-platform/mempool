...
def generate_chest_q16(nb_tx, nb_rx, nb_samples):
    ...
    for k in range(nb_samples):
        ...
        fixed_point = 12
        scaling_factor = 1
        q_pilot_tx = fixed_point_conversion(np.reshape(pilot_tx, [nb_tx]), fixed_point, 1)
        q_pilot_rx = fixed_point_conversion(np.reshape(pilot_rx, [nb_rx]), fixed_point, scaling_factor)
