...
def generate_mimo_mmse_f32(N_tx, N_rx, N_itr):
    ...
    for k in range(N_itr):
        ...
        vH[k,:] = np.column_stack((H.real, H.imag)).astype(np.float32).flatten()
        vG[k,:] = np.column_stack((G.real, G.imag)).astype(np.float32).flatten()
        vy[k,:] = np.column_stack((y.real, y.imag)).astype(np.float32).flatten()
        vx[k,:] = np.column_stack((x.real, x.imag)).astype(np.float32).flatten()
...
def generate_mimo_mmse_f16(N_tx, N_rx, N_itr, randomize):
    ...
    for k in range(N_itr):
        ...
        for i in range(N_beamgroups):
            ...
            sigma = np.column_stack((sigma.real, sigma.imag)).astype(np.float16).flatten()
            H = np.column_stack((H.real, H.imag)).astype(np.float16).flatten()
            G = np.column_stack((G.real, G.imag)).astype(np.float16).flatten()
            y = np.column_stack((y_bg.real, y_bg.imag)).astype(np.float16).flatten()
            ...
            vG[k, (i * 2 * N_tx_itr * N_tx_itr):((i + 1) * 2 * N_tx_itr * N_tx_itr)] = G
            vy[k, :] = y
...
def generate_mimo_mmse_q16(N_tx, N_rx, N_itr):
    ...
    for k in range(N_itr):
        ...
        + 1.j * np.random.randint(-2**(15), 2**(15) - 1, N_rx * N_tx, dtype=np.int16)
        # Create input vector
        ...
        G = np.reshape(np.asarray(G), (N_tx * N_tx), order='C')
        vSigma[k,:] = np.column_stack((sigma.real, sigma.imag)).astype(np.int16).flatten()
...
def main():
    ...
    vSigma, vH, vG, vy, vx, beamgroups = generate_mimo_mmse_f16(N_tx, N_rx, N_itr, 1)
