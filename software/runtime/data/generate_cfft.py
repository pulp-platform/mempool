...
def generate_cfft_q16(N):
    ...
    src = (np.random.randint(-2**(15), 2**(15) - 1, 2 * N, dtype=np.int16)).astype(np.int16)
    ...
    for i in range(N):
        shift = 2**(my_fixpoint)
        complex_src[i] = (src[2 * i].astype(np.csingle) / shift) + 1j * (src[2 * i + 1].astype(np.csingle) / shift)
...
def main():
    ...
    gen_data_header_file(args.outdir, pathlib.Path(__file__).parent.absolute()/"data_cfft_q16.h.tpl", **kwargs)
    ...
    kwargs = {'name': 'data_cfft_radix4_f16',
              ...
              'Log2Len': int(np.log2(Len)),
              'BitrevLen': len(brv_cfft_q16)}
    gen_data_header_file(args.outdir, pathlib.Path(__file__).parent.absolute()/"data_cfft_f16.h.tpl", **kwargs)
