...
void mempool_cholesky_FL_q32p(int32_t *pSrc, int32_t *pL, const uint32_t n,
                              const uint32_t nPE) {
  ...
  for (uint32_t j = 0; j < n; j++) {
    ...
    mempool_cholesky_q32p_FLsqrtsum(pSrc, pL, core_id, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
    // Second part of Cholesky decomposition
    mempool_cholesky_q32p_FLdivisum(pSrc, pL, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
  }
  return;
}

void mempool_cholesky_FR_q32p(int32_t *pSrc, int32_t *pL, const uint32_t n,
                              const uint32_t nPE) {
  ...
  for (uint32_t j = 0; j < n; j++) {
    // First part of Cholesky decomposition
    mempool_cholesky_q32p_FRsqrtsum(pSrc, pL, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
    // Second part of Cholesky decomposition
    mempool_cholesky_q32p_FRdivisum(pSrc, pL, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
  }
  return;
}
...
void mempool_cholesky_fold_q32p(int32_t *pSrcA, int32_t *pSrcB, int32_t *pLL,
  ...
  for (uint32_t j = 0; j < n; j++) {
    // First part of Cholesky decomposition
    mempool_cholesky_q32p_FLsqrtsum(pSrcA, pLL, core_id, j);
    mempool_cholesky_q32p_FRsqrtsum(pSrcB, pLR, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
    // Second part of Cholesky decomposition
    mempool_cholesky_q32p_FLdivisum(pSrcA, pLL, core_id, n, j);
    mempool_cholesky_q32p_FRdivisum(pSrcB, pLR, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
  }
  return;
}

/* Many Cholesky decompositions can be scheduled on different rows and columns
 */
