...
#define CMATMUL_2x2_LOOP                                                       \
  ...
  v2h res00, res01;                                                            \
  v2h res10, res11;                                                            \
  asm volatile("vfcpka.h.s %[res00], %[sum00_real], %[sum00_imag];"            \
...
#define CMATMUL_2x4_LOOP_FOLDED                                                \
  float sum00_real = 0.0f;                                                     \
  float sum01_real = 0.0f;                                                     \
  float sum02_real = 0.0f;                                                     \
  float sum03_real = 0.0f;                                                     \
  ...
  for (j = 0; j < N; j += 2) {                                                 \
    v2h a00 =                                                                  \
        (*(v2h *)&A_folded[2 * ((i + 0) * NUM_BANKS + shift * N + (j + 0))]);  \
    v2h a01 =                                                                  \
        (*(v2h *)&A_folded[2 * ((i + 0) * NUM_BANKS + shift * N + (j + 1))]);  \
    v2h a10 =                                                                  \
        (*(v2h *)&A_folded[2 * ((i + 1) * NUM_BANKS + shift * N + (j + 0))]);  \
    v2h a11 =                                                                  \
        (*(v2h *)&A_folded[2 * ((i + 1) * NUM_BANKS + shift * N + (j + 1))]);  \
    v2h b00 = (*(v2h *)&B[2 * ((j + 0) * P + (k + 0))]);                       \
    v2h b01 = (*(v2h *)&B[2 * ((j + 0) * P + (k + 1))]);                       \
    v2h b02 = (*(v2h *)&B[2 * ((j + 0) * P + (k + 2))]);                       \
    v2h b03 = (*(v2h *)&B[2 * ((j + 0) * P + (k + 3))]);                       \
...
void cmatmul_2x4_folded_f16p(__fp16 const *__restrict__ A,
  ...
  uint32_t j = 0; // loop counter for N
  uint32_t k = 0; // loop counter for P
  uint32_t NUM_BANKS = NUM_CORES * 4;

  //  uint32_t core_id_shuffling = core_id / (N / 4);
  //  uint32_t numThreads_shuffling = numThreads / (N / 4);
  //  if ((core_id % (N / 4)) == 0) {
  //    // Loop over the rows with cores from different cluster locations
  //    for (i = core_id_shuffling; i < M; i += numThreads_shuffling) {
  //      for (j = 0; j < N; j += 4) {
  //        // Copy multiple A matrices in memory
  //        v2h a0 = (*(v2h *)&A[2 * (i * N + j)]);
  //        v2h a1 = (*(v2h *)&A[2 * (i * N + j + 1)]);
  //        v2h a2 = (*(v2h *)&A[2 * (i * N + j + 2)]);
  //        v2h a3 = (*(v2h *)&A[2 * (i * N + j + 3)]);
  //        for (k = 0; k < NUM_BANKS / N; k++) {
  //          (*(v2h *)&A_folded[2 * (i * NUM_BANKS + k * N + j)]) = a0;
  //          (*(v2h *)&A_folded[2 * (i * NUM_BANKS + k * N + j + 1)]) = a1;
  //          (*(v2h *)&A_folded[2 * (i * NUM_BANKS + k * N + j + 2)]) = a2;
  //          (*(v2h *)&A_folded[2 * (i * NUM_BANKS + k * N + j + 3)]) = a3;
  //        }
  //      }
  //    }
  //  }
  ...
  for (k = core_id * 4; k < N * M; k += 4 * numThreads) {
    ...
    for (uint32_t idx_copy = 0; idx_copy < num_copy; idx_copy++) {
      (*(v2h *)&A_folded[2 * (i * NUM_BANKS + j + idx_copy * N)]) = a0;
      (*(v2h *)&A_folded[2 * (i * NUM_BANKS + j + 1 + idx_copy * N)]) = a1;
      (*(v2h *)&A_folded[2 * (i * NUM_BANKS + j + 2 + idx_copy * N)]) = a2;
      (*(v2h *)&A_folded[2 * (i * NUM_BANKS + j + 3 + idx_copy * N)]) = a3;
    }
  }
  mempool_barrier(numThreads);

  uint32_t shift = core_id / N;
  for (k = core_id * 4; k < P; k += 4 * numThreads) {
    for (i = 0; i < M; i += 2) {
