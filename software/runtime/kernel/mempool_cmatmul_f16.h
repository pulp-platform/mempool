...
#define CMATMUL_1x1_LOOP                                                       \
  ...
  for (j = 0; j < N; j++) {                                                    \
    ...
    v2h b = *(v2h *)&B[2 * (j * P + k)];                                       \
    asm volatile(                                                              \
        "pv.shuffle2.h  %[as],       %[a],  %[mask_shuffle];"                  \
        "vfdotpex.s.h   %[sum_imag], %[as], %[b];"                             \
        "xor            %[as],       %[a],  %[mask_sign];"                     \
        "vfdotpex.s.h   %[sum_real], %[as], %[b];"                             \
        : [sum_real] "+r"(sum_real), [sum_imag] "+r"(sum_imag), [as] "+r"(as)  \
...
#define CMATMUL_2x4_LOOP                                                       \
  ...
  asm volatile(                                                                \
      "vfcpka.h.s %[sum00_real], %[sum00_real], %[sum00_imag];"                \
      "vfcpka.h.s %[sum01_real], %[sum01_real], %[sum01_imag];"                \
      "vfcpka.h.s %[sum02_real], %[sum02_real], %[sum02_imag];"                \
      "vfcpka.h.s %[sum03_real], %[sum03_real], %[sum03_imag];"                \
      "vfcpka.h.s %[sum10_real], %[sum10_real], %[sum10_imag];"                \
      "vfcpka.h.s %[sum11_real], %[sum11_real], %[sum11_imag];"                \
      "vfcpka.h.s %[sum12_real], %[sum12_real], %[sum12_imag];"                \
      "vfcpka.h.s %[sum13_real], %[sum13_real], %[sum13_imag];"                \
      : [sum00_real] "+&r"(sum00_real), [sum01_real] "+&r"(sum01_real),        \
        [sum02_real] "+&r"(sum02_real), [sum03_real] "+&r"(sum03_real),        \
        [sum10_real] "+&r"(sum10_real), [sum11_real] "+&r"(sum11_real),        \
        [sum12_real] "+&r"(sum12_real), [sum13_real] "+&r"(sum13_real)         \
