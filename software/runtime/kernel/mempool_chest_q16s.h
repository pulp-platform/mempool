...
static inline void chest_unrolled4_inner_loop(int16_t *pPilotTX, int16_t *pH,
                                              uint32_t nTX, v2s ab, v2s ab_n,
  ...
  for (uint32_t k = 0; k < nSc; k++) {
    for (uint32_t i = 0; i < nRX; i += 4) {
      ...
      asm volatile(
          ...
          :);
      for (uint32_t j = 0; j < nTX; j += 4) {
        chest_unrolled4_inner_loop(pPilotTX, pH, nTX, ab0, ab_n0, i, j);
        chest_unrolled4_inner_loop(pPilotTX, pH, nTX, ab1, ab_n1, i + 1, j);
        chest_unrolled4_inner_loop(pPilotTX, pH, nTX, ab2, ab_n2, i + 2, j);
        chest_unrolled4_inner_loop(pPilotTX, pH, nTX, ab3, ab_n3, i + 3, j);
      }
    }
    pPilotTX += 2 * nTX;
    pPilotRX += 2 * nRX;
