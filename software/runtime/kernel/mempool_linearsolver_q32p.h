...
void mempool_linearsolver_fold_q32p(int32_t *pSrcA, int32_t *pSrcB,
  ...
  for (uint32_t j = 0; j < n; j++) {
    mempool_linearsolver_q32p_FLsqrtsum(pSrcA, pLL, pInA, core_id, j);
    mempool_linearsolver_q32p_FRsqrtsum(pSrcB, pLR, pInB, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
    mempool_linearsolver_q32p_FLdivisum(pSrcA, pLL, pInA, core_id, n, j);
    mempool_linearsolver_q32p_FRdivisum(pSrcB, pLR, pInB, core_id, n, j);
    mempool_log_partial_barrier(2, absolute_core_id, nPE);
  }
