...
static inline void power_profile(int32_t *const a) {
  ...
  const uint32_t NUM_BANKS_PER_TILE = 16;
  uint32_t tile_id = (core_id / NUM_CORES_PER_TILE) % NUM_TILES_PER_GROUP;
  uint32_t remote_group = core_id % NUM_CORES_PER_TILE;
  uint32_t remote_tile = (NUM_TILES_PER_GROUP * remote_group) + ((tile_id + (NUM_TILES_PER_GROUP/2)) % NUM_TILES_PER_GROUP);
  uint32_t remote_bank = (remote_tile * NUM_BANKS_PER_TILE) + ((core_id / NUM_CORES_PER_GROUP) * (NUM_BANKS_PER_TILE/NUM_CORES_PER_TILE));
  ...
  __asm__ volatile(
      ...
      : [im_0] "r"(im_0), [num_loops] "I"(num_loops), [op_a0] "r"(op_a0),
        ...
        [op_a7] "r"(op_a7), [a_local] "r"(addr_local), [a_remote] "r"(addr_remote)
      : "memory");
}
