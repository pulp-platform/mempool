...
// 64-point radix-4 DIT FFT with only xqueue extension used for communication
...
void systolic_init(uint32_t const *tile_mapping, uint32_t const *core_mapping, int32_t *const shuffling_order) {
  ...
  for (uint32_t y = 1; y < NUM_STAGE; ++y) {
    for (uint32_t x = 0; x < PE_PER_STAGE; ++x) {
      ...
      tile_offset = tile_id * SEQ_MEM_SIZE;
      core_offset = (core_id % 4) * 4;
      queues_input_0[y-1][x] = &__seq_start + tile_offset + core_offset + 0;
      queues_input_1[y-1][x] = &__seq_start + tile_offset + core_offset + 1;
      queues_input_2[y-1][x] = &__seq_start + tile_offset + core_offset + 2;
      queues_input_3[y-1][x] = &__seq_start + tile_offset + core_offset + 3;
      ++grid_pos;
    }
  }
...
void systolic_first_fft_pe(uint32_t stage_idx, uint32_t idx_in_stage){
  int32_t I0, I1, I2, I3;
  int32_t O0, O1, O2, O3;
  int32_t *queue_prev_0;
  int32_t *queue_prev_1;
  int32_t *queue_prev_2;
  int32_t *queue_prev_3;
  int32_t *queue_next_0;
  int32_t *queue_next_1;
  int32_t *queue_next_2;
  int32_t *queue_next_3;
  ...
  // Assign queues
  queue_prev_0 = queues_input_0[stage_idx-1][idx_in_stage];
  queue_prev_1 = queues_input_1[stage_idx-1][idx_in_stage];
  queue_prev_2 = queues_input_2[stage_idx-1][idx_in_stage];
  queue_prev_3 = queues_input_3[stage_idx-1][idx_in_stage];
  ...
  queue_pop(queue_prev_0, &I0);
  queue_pop(queue_prev_1, &I1);
  queue_pop(queue_prev_2, &I2);
  queue_pop(queue_prev_3, &I3);
...
#ifndef ASM
  v2s s2 = {2, 2};
  B = (v2s) _SRA2(*(v2s *)&I1,s2);
  D = (v2s) _SRA2(*(v2s *)&I3,s2);
  A = (v2s) _SRA2(*(v2s *)&I0,s2);
  C = (v2s) _SRA2(*(v2s *)&I2,s2);
...
void systolic_end_pe(uint32_t idx_in_stage, int16_t *const vector_output, int32_t *const shuffling_order){
  ...
  // Assign queues
  queue_prev_0 = queues_input_0[2][idx_in_stage];
  queue_prev_1 = queues_input_1[2][idx_in_stage];
  queue_prev_2 = queues_input_2[2][idx_in_stage];
  queue_prev_3 = queues_input_3[2][idx_in_stage];
