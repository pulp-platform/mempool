...
DEFINES += -DNUM_BANKS=$(shell awk 'BEGIN{print $(banking_factor)*($(num_cores)+$(num_redmule_tiles)*($(num_cores_per_tile)-1))}')
DEFINES += -DNUM_CORES_PER_GROUP=$(shell awk 'BEGIN{print int($(num_cores)/$(num_groups))}')
DEFINES += -DNUM_TILES_PER_GROUP=$(shell awk 'BEGIN{print (int(($(num_cores)-$(num_redmule_tiles))/$(num_cores_per_tile))+$(num_redmule_tiles))/$(num_groups)}')
