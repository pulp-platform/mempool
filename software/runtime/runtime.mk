...
DEFINES += -DNUM_CLUSTERS=$(num_clusters)
DEFINES += -DNUM_CORES_PER_TILE=$(num_cores_per_tile)
DEFINES += -DBANKING_FACTOR=$(banking_factor)
DEFINES += -DNUM_CORES_PER_CLUSTER=$(shell awk 'BEGIN{print $(num_cores)/$(num_clusters)}')
DEFINES += -DNUM_TILES_PER_CLUSTER=$(shell awk 'BEGIN{print ($(num_cores)/$(num_clusters))/$(num_cores_per_tile)}')
...
DEFINES += -DLOG2_NUM_CORES_PER_GROUP=$(shell awk 'BEGIN{print log($(num_cores)/$(num_groups))/log(2)}')
DEFINES += -DLOG2_NUM_CORES_PER_CLUSTER=$(shell awk 'BEGIN{print log($(num_cores)/$(num_clusters))/log(2)}')
DEFINES += -DBOOT_ADDR=$(boot_addr)
DEFINES += -DL1_BANK_SIZE=$(l1_bank_size)
DEFINES += -DLOG2_L1_SIZE_PER_CLUSTER=$(shell awk 'BEGIN{print log($(l1_bank_size)*$(banking_factor)*$(num_cores)/$(num_clusters))/log(2)}')
