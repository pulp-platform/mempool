...
# x: (4,), H: (8, 4), y: (8,), N: ()
    ...
    # MMSE estimator
    H_h = H.conj().T #4,8
    HH_h = np.matmul(H,H_h) #8,8
    G = np.linalg.inv(HH_h + N * np.eye(H.shape[0])) #8,8
    G = np.matmul(H_h, G) #4,8
    GH = np.matmul(G,H) # 4,4
    Gy = np.matmul(G,np.expand_dims(y,axis=-1)) #4,1
    d = np.diag(GH) # 4
    Gy = np.squeeze(Gy) # 4
    xhat = Gy/d #4
    ...
    N = 1/d - 1
    N = N.real.astype(my_type)
    # print(f"H: {H.shape}, x: {x.shape}, y: {y.shape}, N: {N.shape}, H_h: {H_h.shape}, HH_h: {HH_h.shape}, G: {G.shape}, GH: {GH.shape}, Gy: {Gy.shape}, d: {d.shape}, xhat: {xhat.shape}")
    # Type cast
...
def mmse(x, H, y, N, my_type):
    ...
    # H_hy = np.matmul(H_h, np.expand_dims(y,axis=1))
    # xhat = np.matmul(np.linalg.inv(G), H_hy)
...
def generate_mimo_transmission_f16(
        N_tx, N_rx, N_itr, symbols, channel_type, SNRdB, batch_size):
    # Create input vector
    idx = np.random.randint(0, len(symbols), size=[batch_size,N_tx])
    x = symbols[idx] # shape (batch_size, N_tx)

    # 16QAM
    # x, b = sionna_pool.get_random_symbols(batch_size=batch_size, num_ut=N_tx, num_bits_per_symbol = 4)
    # print(f"x: shape = {x.shape}, dtype = {x.dtype}, xx: shape = {xx.shape}, dtype = {xx.dtype}")
    # print(f"x: shape = {x.shape}, dtype = {x.dtype}, b: shape = {b.shape}, dtype = {b.dtype}")


    N_rx_dual_pol = int(N_rx/2)
    # Generate channel and noise
    if channel_type == 'rayleigh': 
        # Generate Rayleigh fading channel
        scale = 1.0
        H = np.sqrt(np.random.chisquare(2, [batch_size, N_rx, N_tx])) + 1j * \
            np.sqrt(np.random.chisquare(2, [batch_size, N_rx, N_tx]))
        Es = np.mean(abs(symbols)**2)
        Eh = (np.linalg.norm(H, 'fro',axis=(1,2))**2) / N_rx
    elif channel_type == 'random':
        H = np.sqrt(0.5) * \
            (np.random.normal(0, 1, [batch_size,N_rx, N_tx]) + 1j *
            np.random.normal(0, 1, [batch_size,N_rx, N_tx]))
        Es = np.mean(abs(symbols)**2)
        Eh = (np.linalg.norm(H, 'fro',axis=(1,2))**2) / N_rx
    # elif channel_type == 'uma': # Use with OFDM
    #     H = sionna_pool.channel_gen(scenario="uma", carrier_frequency=3.5e9, num_ut=N_tx, batch_size=batch_size,
    #             rx_ant_rows_cols=[1,N_rx_dual_pol], min_ut_velocity=0, max_ut_velocity=0)
    #     Es = np.mean(abs(symbols)**2)
    #     Eh = (np.linalg.norm(H, 'fro',axis=(1,2))**2) / N_rx
    # elif channel_type == 'umi': # Use with OFDM
    #     H = sionna_pool.channel_gen(scenario="umi", carrier_frequency=3.5e9, num_ut=N_tx,batch_size=batch_size,
    #             rx_ant_rows_cols=[1,N_rx_dual_pol], min_ut_velocity=0, max_ut_velocity=0)
    #     Es = np.mean(abs(symbols)**2)
    #     Eh = (np.linalg.norm(H, 'fro',axis=(1,2))**2) / N_rx
    # elif channel_type == 'rma': # Use with OFDM
    #     H = sionna_pool.channel_gen(scenario="rma", carrier_frequency=3.5e9, num_ut=N_tx,batch_size=batch_size,
    #             rx_ant_rows_cols=[1,N_rx_dual_pol], min_ut_velocity=0, max_ut_velocity=0)
    #     Es = np.mean(abs(symbols)**2)
    #     Eh = (np.linalg.norm(H, 'fro',axis=(1,2))**2) / N_rx
    # elif channel_type == 'flatfading': # It generates complex Gaussian matrices, (Flat Fading)
    #     H = sionna_pool.flat_fading_channel(num_tx_ant=4, num_rx_ant=4, batch_size=batch_size)
    #     Es = np.mean(abs(symbols)**2)
    #     Eh = (np.linalg.norm(H, 'fro',axis=(1,2))**2) / N_rx
    else:
    ...
    vN16 = np.empty((2 * N_tx, batch_size), dtype=np.float16)
    # vN16 = np.empty((1 * N_tx, batch_size), dtype=np.float16)
    vy16 = np.empty((2 * N_rx, batch_size), dtype=np.float16)
    vH16 = np.empty((2 * N_tx * N_rx, batch_size), dtype=np.float16)
    vx16 = np.empty((2 * N_tx, batch_size), dtype=np.float16)
    vxhat16 = np.empty((2 * N_tx, batch_size), dtype=np.float16)
    # Golden model
    vx64 = np.empty((2 * N_tx, batch_size), dtype=np.float64)
    vxhat64 = np.empty((2 * N_tx, batch_size), dtype=np.float64)
    # This Loop can be removed for speed-up
    for iBatch in range(batch_size):
        ...
        # Noise variance
        N = 0.5 * Es * Eh_ * 10**(-SNRdB / 10)
        n = (np.random.normal(0, np.sqrt(N), N_rx) + 1j *
            np.random.normal(0, np.sqrt(N), N_rx))
        N = N / Es

        # Channel propagation
        y = np.dot(H_, x_) + n.flatten()
        # MMSE estimator
        N64, H64, y64, x64, xhat64 = mmse(x_, H_, y, N, np.float64)
        N16, H16, y16, x16, xhat16 = mmse(x_, H_, y, N, np.float16)
        ...
        vx16[:, iBatch] = x16
        vx64[:, iBatch] = x64
        vxhat16[:, iBatch] = xhat16
        vxhat64[:, iBatch] = xhat64
...
def generate_mimo_transmission_f16_old(
    ...
    if channel_type == 'rayleigh':
        ...
        Es = np.mean(abs(symbols)**2)
        Eh = (np.linalg.norm(H, 'fro')**2) / N_rx
    elif channel_type == 'uma':
        H = sionna_pool.channel_gen(scenario="uma", carrier_frequency=3.5e9, num_ut=N_tx,
                rx_ant_rows_cols=[1,2], min_ut_velocity=0, max_ut_velocity=0, batch_size=1)
        Es = np.mean(abs(symbols)**2)
        Eh = (np.linalg.norm(H, 'fro')**2) / N_rx
    elif channel_type == 'umi':
        H = sionna_pool.channel_gen(scenario="umi", carrier_frequency=3.5e9, num_ut=N_tx,
                rx_ant_rows_cols=[1,2], min_ut_velocity=0, max_ut_velocity=0, batch_size=1)
        Es = np.mean(abs(symbols)**2)
        Eh = (np.linalg.norm(H, 'fro')**2) / N_rx
    elif channel_type == 'rma':
        H = sionna_pool.channel_gen(scenario="rma", carrier_frequency=3.5e9, num_ut=N_tx,
                rx_ant_rows_cols=[1,2], min_ut_velocity=0, max_ut_velocity=0, batch_size=1)
        Es = np.mean(abs(symbols)**2)
        Eh = (np.linalg.norm(H, 'fro')**2) / N_rx
    else:
...
def main():
    ...
    for iSNR in range(0, len(vSNRdB)):
        ...
        for iMC in range(0, N_itr, N_batch):
            ...
            #     output_mmse = generate_mimo_transmission_f16_old(
            ...
            # Random BER iterations (Batched)
            # batched to speedup the generation process of new channel models
            output_mmse = generate_mimo_transmission_f16(
                N_tx, N_rx, N_batch, const.symbols, channel_type, SNRdB, batch_size=N_batch)
            vN16 = output_mmse["vN16"]
            vy16 = output_mmse["vy16"]
            vH16 = output_mmse["vH16"]
            vx16 = output_mmse["vx16"]
            vx64 = output_mmse["vx64"]
            vxhat16 = output_mmse["vxhat16"]
            vxhat64 = output_mmse["vxhat64"]
            

            # Collect decoded symbols
            vxhat[0, :, :] = vxhat64
            print(f'N_batch:{N_batch}, vy16:{vy16.shape}, vH16:{vH16.shape}, vN16:{vN16.shape}')
            # ----------------------------------------------------------------
            # BANSHEE CALL
            # ----------------------------------------------------------------
            if run_banshee:
                for iPrec, (precision, flag) in enumerate(precisions[1:]):
                    ...
                    file = os.path.join(DATA_DIR,'banshee_out_0.txt')
                    with open(file, 'w') as f:
            ...
            # ----------------------------------------------------------------
            # b = output_mmse["b"]
            # vbit_xhat = sionna_pool.get_bits(vxhat,vN16,num_bits_per_symbol=4)
            # for iPrec in range(0, len(precisions)):
            #     vTBE[iPrec][iSNR] += sum(vbit_xhat.reshape((-1,))-b)
