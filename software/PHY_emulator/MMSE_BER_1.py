...
def main():
   ...
   # Instanciate OFDMMIMO class
    ofdm_mimo = sionna_pool.OFDMMIMO(scenario=channel_type, perfect_csi=True, carrier_frequency=carrier_frequency,fft_size=fft_size,
    ...
    for iSNR in range(0, len(vSNRdB)):
        ...
        for iMC in range(0, max_it):
            ...
            # print(f'tbe:{tbe}, num_bits:{num_bits}, ber:{ber}, tbe/num_bits:{tbe/num_bits}')
            # print(f'SNR:{SNRdB}, iMC:{iMC}, ber:{ber}')
            # print(f'no_db:{no_db}')
            ...
            # b_hat = b_hat.numpy().astype(bits_dtype).flatten()
            # b = b.numpy().astype(bits_dtype).flatten()


            # Collect decoded symbols
            # vxhat[0, :, :] = vxhat_gm
            # vbhat[0,:]= b_hat.numpy().astype(bits_dtype).flatten()
            
            # print(f'vy16:{vy16.shape}, vH16:{vH16.shape}, vN16:{vN16.shape}')
            # print(f's:{s.shape}, {s.numpy()[0,:]} \n\n')
            # print(f'vN16:{vN16[:,0]}')

            # # test local mmse ------ (debug) -------
            # vN16 = np.empty((2 * N_tx, N_batch * num_symbols), dtype=np.float16)
            # vy16 = np.empty((2 * N_rx, N_batch * num_symbols), dtype=np.float16)
            # vH16 = np.empty((2 * N_tx * N_rx, N_batch * num_symbols), dtype=np.float16)
            # for ii in range( N_batch * num_symbols):
            #     y_ = y.numpy()[ii]
            #     H_ = np.reshape(H.numpy()[ii],(N_rx,N_tx))
            #     N_ = s.numpy().real[ii,0]
                


            #     Nl, Hl, yl, xhat_l = mmse(H_, y_, N_, gm_dtype_real)


            #     N16, H16, y16, xhat16 = mmse(H_, y_, N_, np.float16)
            ...
            # BANSHEE CALL
            # ----------------------------------------------------------------
            # banshee_batch_size = N_batch * num_symbols
            banshee_batch_size = vy16.shape[1] # to support simple_mimo too
            # print(f'banshee_batch_size:{banshee_batch_size}, vy16:{vy16.shape}, vH16:{vH16.shape},  vN16:{vN16.shape}')
            if run_banshee:
                for iPrec, (precision, flag) in enumerate(precisions[1:]):
                    ...
                    # print(f'iPerc:{iPrec}, bans0:{xhat_[0]}')
                    # print(f'bans1:{xhat_[1]}')
                    # print(f'xhat_:{xhat_.shape}, xhat_.dtype')
                    ...
                    # print(f'iperc:{iPrec}, tbe:{tbe}, num_bits:{num_bits}, ber:{ber}, tbe/num_bits:{tbe/num_bits}')

                    vTBE[iPrec+1,iSNR] += tbe
                    vTB[iPrec+1,iSNR] += num_bits
                    # print(f'b:{b.shape}, b_hat:{b_hat.shape}, b_hat_:{b_hat_.shape}')

                    # vxhat[iPrec + 1, :, :] = cast_reshape_to_2D_real(xhat_.numpy(), dtype=gm_dtype_real)
                    # vbhat[iPrec + 1,:]= b_hat_.numpy().astype(bits_dtype).flatten()
            # ----------------------------------------------------------------
            # ----------------------------------------------------------------
            ...
            # ----------------------------------------------------------------
            # BER COMPUTATION (for one batch)
            # ----------------------------------------------------------------
            # for iPrec in range(0, len(precisions)):
            #     vTBE[iPrec][iSNR] += np.sum( vbhat[iPrec,:]!=b )
        ...
        # total_bits1 = (N_tx * num_bits_per_symbol * num_data_symbols * N_itr * N_batch)
        # print(f'total_bits:{total_bits}, total_bits1:{total_bits1}')
