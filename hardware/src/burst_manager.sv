...
module burst_manager #(
    ...
    for (int unsigned i = 0; i < NrInOut; i++) begin
      if (req_valid_i[i] && req_payload_i[i].rburst.burst) begin
        ...
        if (prearb_ready[i]) begin
          // request is picked by arbitrator and fifo, mark as accepted
          ready_mask[i]   = 1'b1;
        end else begin
  ...
  // logic [TCDMAddrWidth-1:0] [NrInOut-1:0] gen_addr;
  ...
  always_comb begin : request_generator
    ...
    // The mask will only be available for one cycle, do not pass it from q
    group_mask  = '0;
    ...
    // req_ready_o   = req_ready_i & (~burst_mask_q);
