...
`ifndef TARGET_SYNTHESIS
  always_ff @(posedge rst_ni) begin;
    if(rst_ni && (tile_id_i == 1)) begin
      $display("Tile     %8d master_req           %8d", tile_id_i, $bits(tcdm_master_req_t)*NumGroups+NumSubGroupsPerGroup + 2*NumGroups+NumSubGroupsPerGroup);
      $display("Tile     %8d master_resp          %8d", tile_id_i, $bits(tcdm_master_resp_t)*NumGroups+NumSubGroupsPerGroup + 2*NumGroups+NumSubGroupsPerGroup);
      $display("Tile     %8d slave_req            %8d", tile_id_i, $bits(tcdm_slave_req_t)*NumGroups+NumSubGroupsPerGroup + 2*NumGroups+NumSubGroupsPerGroup);
      $display("Tile     %8d slave_resp           %8d", tile_id_i, $bits(tcdm_slave_resp_t)*NumGroups+NumSubGroupsPerGroup + 2*NumGroups+NumSubGroupsPerGroup);
      $display("Tile     %8d bank_req             %8d", tile_id_i, (3+$bits(strb_t)+$bits(data_t)+$bits(amo_t)+$bits(tile_addr_t)+$bits(bank_metadata_t))*NumBanksPerTile);
      $display("Tile     %8d bank_resp            %8d", tile_id_i, (2+$bits(data_t)+$bits(bank_metadata_t))*NumBanksPerTile);
    end
  end
`endif
