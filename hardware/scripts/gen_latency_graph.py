...
parser.add_argument("--section", "-s", help="Index of section for analysis.")
...
for files in os.listdir(path):
    if files.endswith(ext):
        ...
        for i in range(len(snitch_load_region)):
            ...
            snitch_global_load_latency = np.append(
                snitch_global_load_latency, latency[global_load_indices]
            )
        ...
        plt.text(
            0.95,
            0.65,
            f"50th percentile: {percentile_50}",
            ha="right",
            va="top",
            transform=plt.gca().transAxes,
            fontsize=12,
        ...
        plt.text(
            0.95,
            0.55,
            f"99th percentile: {percentile_99}",
            ha="right",
            va="top",
            transform=plt.gca().transAxes,
            fontsize=12,
