                ...
                if (memory_address << 2) >= 0x0:
                    ...
                    if burst_address not in burst_data:
                        burst_data[burst_address] = {
                            ...
                            'last_wr': last_wr_cyc,
                            'last': last_acc_cyc,
                            'mem_addr': (burst_address * burst_size) << 2, # each word is 32bit width
                            'rd_cycles': rd_cycles, 'wr_cycles': wr_cycles
                        }
                    else:
                        burst_entry = burst_data[burst_address]
                        burst_entry['init'] = min(burst_entry['init'], ini_cyc)
                        burst_entry['last_rd'] = max(burst_entry['last_rd'], last_rd_cyc)
                        burst_entry['last_wr'] = max(burst_entry['last_wr'], last_wr_cyc)
                        burst_entry['last'] = max(burst_entry['last'], last_acc_cyc)
...
def hex_format(x, pos):
    """The two args are the value and tick position. Format the number as hex with '0x' prefix."""
    return f'0x{int(x):X}'
...
def calc_completion_percentages(cycles, last_cycles):
    """
    Calculate the percentage of bursts that have completed by each cycle in cycles list,
    considering only bursts with 'last_acc_cyc' greater than 2000.
    """
    ...
    for cycle in cycles:
        completed = np.searchsorted(filtered_cycles_sorted, cycle, side='right')
        percentages.append((completed / total_bursts) * 100 if total_bursts > 0 else 0)
    return percentages
...
def custom_xaxis_format(cycles, percentages, tick_spacing):
    """
    Custom formatter to show both cycle number and completion percentage on x-axis ticks.
    """
...
def plot_lifetime(df, output_file, file_title, burst_size):
    ...
    # cmap = get_cmap('jet')
    norm = Normalize(vmin=df['Init Cycle'].min(), vmax=df['Last Access Cycle'].max())

    legend_added = {'write': False, 'next_read': False, 'last_read': False}  # To control legend entries

    # Calculate completion percentages for x-axis ticks
    tick_spacing_x = round_up_to_msb(df['Last Access Cycle'].max()) / 10  # Defines the spacing between ticks
    tick_vals_x = np.arange(
                        start=min(0, ((df['Init Cycle'].min() // tick_spacing_x) * tick_spacing_x)), 
                        stop=(df['Last Access Cycle'].max() // tick_spacing_x + 2) * tick_spacing_x, 
                        step=tick_spacing_x)
    x_ticks = tick_vals_x
    percentages = calc_completion_percentages(x_ticks, df['Last Access Cycle'].values)
    ...
    markers = {
        ...
        'next_read': {'line': '-', 'marker': '^', 'label': 'Last Access is Read'},
        # Solid line with round points for read completion
        'last_read': {'line': '-', 'marker': 'o', 'label': 'Last Access is Read'},
        # Dotted line with 'x' points for write completion
        'write': {'line': '--', 'marker': 'x', 'label': 'Last Access is Write'}
    }
    ...
    for _, row in df.iterrows():
        ...
        for i, wr_cyc in enumerate(wr_cycles):
            ...
            next_wr_cyc = wr_cycles[i+1] if i+1 < len(wr_cycles) else float('inf')
            next_rd_cyc = min([rd for rd in rd_cycles if wr_cyc < rd < next_wr_cyc], default=wr_cyc)
            last_rd_cyc = max([rd for rd in rd_cycles if wr_cyc < rd < next_wr_cyc], default=wr_cyc)
            ...
            if next_rd_cyc > wr_cyc and next_rd_cyc < last_rd_cyc:
                plt.plot(next_rd_cyc,
                         ...
                         markeredgewidth=0.2,
                         label='Next Read Point' if not legend_added['next_read'] else "")
                # Update the legend entry management
            ...
            if last_rd_cyc > wr_cyc:
                plt.plot(last_rd_cyc,
                         ...
                         markeredgecolor='grey',
                         markeredgewidth=0.2,
                         label='Last Read Point' if not legend_added['last_read'] else "")
                # Update the legend entry management
    ...
    # ax = plt.gca()
    # # ax.set_ylim([df['Memory Address'].min() - burst_size, df['Memory Address'].max() + burst_size * 2])
    
    # # Set custom y-axis tick locations and format them as hexadecimal
    # tick_spacing = burst_size * burst_size  # Defines the spacing between ticks
    # tick_vals = np.arange(start=(df['Memory Address'].min() // tick_spacing) * tick_spacing, 
    ...
    ax = plt.gca()
    # ax.set_ylim([df['Memory Address'].min() - burst_size, df['Memory Address'].max() + burst_size])
    ax.set_xlim([min(0,((df['Init Cycle'].min() // tick_spacing_x) * tick_spacing_x)),
    ...
    ax.xaxis.set_major_formatter(FuncFormatter(custom_xaxis_format(x_ticks, percentages, tick_spacing_x)))
...
def main(args):
    ...
    if not df.empty:
        last_write_cyc = df['Last Write Cycle'].max()
        last_read_cyc = df['Last Read Cycle'].max()
        print(f"last write cyc: {last_write_cyc}, last read cyc: {last_read_cyc}")
        output_dir = 'plots/4'
...
if __name__ == "__main__":
    ...
    parser.add_argument('file_path', type=str, help='The file path of the data file to analyze')
    parser.add_argument('app', type=str, help='The app name of the data file to analyze')
    parser.add_argument('burst_size', type=int, help='Burst size in bytes for data aggregation')
