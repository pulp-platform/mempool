...
def plot_input_utilization(df, plt_path):
    ...
    plt.bar(
        ...
        idle_perc,
        bar_width,
        bottom=hsk_perc + hol_stall_perc + man_stall_perc,
        label=f"Idle({idle_tot_perc}%)",
        color="gray",
    )
    plt.bar(
        index,
        hsk_perc,
        bar_width,
        bottom=hol_stall_perc + man_stall_perc,
        label=f"In Use({hsk_tot_perc}%)",
        color="green",
    )
    plt.bar(
        index,
        hol_stall_perc,
        bar_width,
        bottom=man_stall_perc,
        label=f"HoL Stall({hol_stall_tot_perc}%)",
        color="orange",
    )
    plt.bar(
        index,
        man_stall_perc,
        bar_width,
        label=f"Mandatory Stall({man_stall_tot_perc}%)",
        color="red",
    )
...
def plot_output_congestion(df, plt_path):
    ...
    plt.bar(
        ...
        clear_perc,
        bar_width,
        bottom=dir0_cong_perc
        + dir1_cong_perc
        + dir2_cong_perc
        + dir3_cong_perc
        + dir4_cong_perc,
        label=f"Clear({clear_tot_perc}%)",
        color="gray",
    )
    plt.bar(
        index,
        dir0_cong_perc,
        bar_width,
        bottom=dir1_cong_perc
        + dir2_cong_perc
        + dir3_cong_perc
        + dir4_cong_perc,
        label=f"Eject({dir0_cong_tot_perc}%)",
        color="brown",
    )
    plt.bar(
        index,
        dir1_cong_perc,
        bar_width,
        bottom=dir2_cong_perc + dir3_cong_perc + dir4_cong_perc,
        label=f"North({dir1_cong_tot_perc}%)",
        color="yellow",
    )
    plt.bar(
        index,
        dir2_cong_perc,
        bar_width,
        bottom=dir3_cong_perc + dir4_cong_perc,
        label=f"East({dir2_cong_tot_perc}%)",
        color="deepskyblue",
    )
    plt.bar(
        index,
        dir3_cong_perc,
        bar_width,
        bottom=dir4_cong_perc,
        label=f"South({dir3_cong_tot_perc}%)",
        color="orange",
    )
    plt.bar(
        index,
        dir4_cong_perc,
        bar_width,
        label=f"West({dir4_cong_tot_perc}%)",
        color="blue",
    )
    plt.xticks([])
...
if level == "system":
    df_pruned = df.drop(
        columns=[
            ...
            "end_cycle",
            "max_stall_cyc_num",
            "out_dir0_cong_cyc_num",
            "out_dir1_cong_cyc_num",
            "out_dir2_cong_cyc_num",
            "out_dir3_cong_cyc_num",
            "out_dir4_cong_cyc_num",
        ]
    )
...
if level == "group":
    for group in range(16):
        ...
        df_congestion_grouped = df_congestion.groupby(
            "start_cycle", as_index=False
        ).agg(
            {
        ...
        for dir in range(5):
            ...
            if dir == 0:
                plt_path = os.path.join(
                    plot_path, f"group_{group}_utilization_inject.png"
                )
            elif dir == 1:
                plt_path = os.path.join(
                    plot_path, f"group_{group}_utilization_north.png"
                )
            elif dir == 2:
                plt_path = os.path.join(
                    plot_path, f"group_{group}_utilization_east.png"
                )
            elif dir == 3:
                plt_path = os.path.join(
                    plot_path, f"group_{group}_utilization_south.png"
                )
            else:
                plt_path = os.path.join(
                    plot_path, f"group_{group}_utilization_west.png"
                )
            plot_input_utilization(df_grouped, plt_path)
    ...
    for tile in range(16):
        ...
        for port in range(2):
            ...
            df_pruned = df_port.drop(
                columns=["GROUP", "TILE", "PORT", "DIR", "end_cycle"]
            )
            df_grouped = df_pruned.groupby("start_cycle", as_index=False).agg(
                {
                    "in_vld_cyc_num": "sum",
            ...
            plt_path = os.path.join(
                plot_path, f"tile_{tile}_port_{port}_utilization.png"
            )
            plot_input_utilization(df_grouped, plt_path)
    ...
    for tile in range(16):
        ...
        for port in range(2):
            ...
            plt_path = os.path.join(
                plot_path,
                f"group_{group_sel}_tile_{tile}_port_{port}_congestion.png",
            )
            plot_output_congestion(df_congestion_grouped, plt_path)
            for dir in range(5):
                ...
                if dir == 0:
                    plt_path = os.path.join(
                        plot_path,
                        f"group_{group_sel}_tile_{tile}_port_{port}_"
                        "utilization_inject.png",
                    )
                elif dir == 1:
                    plt_path = os.path.join(
                        plot_path,
                        f"group_{group_sel}_tile_{tile}_port_{port}_"
                        "utilization_north.png",
                    )
                elif dir == 2:
                    plt_path = os.path.join(
                        plot_path,
                        f"group_{group_sel}_tile_{tile}_port_{port}_"
                        "utilization_east.png",
                    )
                elif dir == 3:
                    plt_path = os.path.join(
                        plot_path,
                        f"group_{group_sel}_tile_{tile}_port_{port}_"
                        "utilization_south.png",
                    )
                else:
                    plt_path = os.path.join(
                        plot_path,
                        f"group_{group_sel}_tile_{tile}_port_{port}_"
                        "utilization_west.png",
                    )
                plot_input_utilization(df_grouped, plt_path)
